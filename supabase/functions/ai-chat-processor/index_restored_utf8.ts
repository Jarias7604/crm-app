...
/**
 * ≡ƒÆÄ V59: THE RESTORED MASTERPIECE
...
async function generateQuotePDF(cot: any, company: any, creator: any, lead: any, supabase: any) {
    try {
        ...
        const drawFooter = async (p: any) => {
            ...
            const rText2 = (company?.address || 'San Salvador, El Salvador') + (company?.phone ? `  ΓÇó  ${company.phone}` : '');
        ...
        const phone = company?.phone ? `  ΓÇó  ${company.phone}` : '';
        ...
        // Header Right
        drawRightText(page, 'COTIZACI├ôN OFICIAL', rm, 14, 7, fBold, C_BLUE);
        ...
        // Dates
        drawRightText(page, 'FECHA EMISI├ôN', rm - 12, 32, 6, fBold, C_GRAY_TEXT);
        ...
        page.drawText('DESCRIPCI├ôN DEL SERVICIO', { x: mmToPt(m + 2), y: invY(tY + 3.5), size: 7, font: fBold, color: C_GRAY_TEXT });
        drawRightText(page, 'INVERSI├ôN (USD)', rm - 2, tY + 3.5, 7, fBold, C_GRAY_TEXT);
        ...
        const addRow = (iconType: string, title: string, subtitle: string, price: number, isOneTime = false) => {
            ...
            if (isOneTime) {
                ...
                page.drawText('PAGO ├ÜNICO', { x: mmToPt(m + 17) + tw + 7, y: invY(tY + 2.8), size: 5, font: fBold, color: rgb(154 / 255, 52 / 255, 18 / 255) });
            }
