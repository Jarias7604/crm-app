...
/**
 * ≡ƒÄ¿ SERVER-SIDE PDF GENERATOR (Deno/Edge Compatible)
...
async function generateQuotePDF(quote: any, supabase: any): Promise<string> {
    try {
        ...
        page.drawText(`Volumen: ${quote.volumen_dtes} DTEs/a├▒o`, { x: 60, y: y - 2, size: 10, font: font, color: colorGray });
...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const pricingContext = (pricingItems || []).map(item => {
            return `- [${item.tipo.toUpperCase()}] ${item.nombre}: $${item.precio_anual}/a├▒o ($${item.precio_mensual}/mes). Pago ├Ünico Setup: $${item.costo_unico || 0}. Rango DTEs: ${item.min_dtes}-${item.max_dtes}.`;
        }).join('\n');
        ...
        if (phoneMatch) {
            ...
            if (existingLead && existingLead.id !== currentConv?.lead_id) {
                ...
                - ACCI├ôN: Antes de guardar, pregunta amablemente: "┬┐Ese n├║mero es tuyo o es de la persona [Nombre del anterior]?", o algo similar para confirmar si es la misma persona con un nombre diferente o alguien m├ís.
                `;
            }
        }
        ...
        No des el PDF sin antes tener: ≡ƒæñ Nombre del contacto, ≡ƒÅ¢∩╕Å Nombre de la Empresa y ≡ƒôä Volumen de facturaci├│n.
        
        [TRIGGERS DE SISTEMA - OBLIGATORIO]
        Si el cliente califica (Nombre, Empresa y Volumen), debes incluir este bloque AL FINAL de tu respuesta, separado por una l├¡nea:
        [AUTO_QUOTE_TRIGGER]: {"dte_volume": TOTAL_ANUAL_CALCULADO, "plan_id": "ID_OPCIONAL"}
        
        S├ë UN VENDEDOR SENIOR: Efectivo, veraz y r├ípido con los documentos.
        `;
