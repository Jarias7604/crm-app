-- Migration to automatically fall back to lead.value when lead.closing_amount is NULL or 0 in dashboard stats
...
BEGIN
    ...
    SELECT json_build_object(
        ...
        'salesTrend', (SELECT COALESCE(json_agg(json_build_object('date', day, 'amount', amount, 'count', count)), '[]'::json) FROM (
            SELECT DATE_TRUNC('day', internal_won_date)::DATE as day, COALESCE(SUM(COALESCE(closing_amount, value, 0)), 0) as amount, COUNT(*) as count 
            FROM public.leads 
