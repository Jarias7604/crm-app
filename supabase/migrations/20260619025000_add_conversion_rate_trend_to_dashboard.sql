-- Migration to add monthly conversion rate trend to dashboard stats salesTrend
...
BEGIN
    ...
    SELECT COUNT(*), COALESCE(SUM(COALESCE(closing_amount, value, 0)), 0), COALESCE(SUM(value), 0)
    INTO v_curr_won_count, v_curr_won_amount, v_curr_won_potential
    ...
    WITH mc AS (
        SELECT 
            m.month_date,
            COALESCE(c.created_count, 0) as total_leads,
            COALESCE(w.won_count, 0) as won_leads
        FROM (
            SELECT DISTINCT DATE_TRUNC('month', created_at)::DATE as month_date 
            FROM public.leads 
            WHERE company_id = p_company_id
            UNION
            SELECT DISTINCT DATE_TRUNC('month', internal_won_date)::DATE as month_date 
            FROM public.leads 
            WHERE company_id = p_company_id AND internal_won_date IS NOT NULL
        ) m
        LEFT JOIN (
            SELECT DATE_TRUNC('month', created_at)::DATE as month_date, COUNT(*) as created_count
            FROM public.leads
            WHERE company_id = p_company_id AND (p_assigned_to IS NULL OR assigned_to = p_assigned_to)
            GROUP BY 1
        ) c ON c.month_date = m.month_date
        LEFT JOIN (
            SELECT DATE_TRUNC('month', internal_won_date)::DATE as month_date, COUNT(*) as won_count
            FROM public.leads
            WHERE company_id = p_company_id AND status IN ('Cerrado', 'Cliente') AND (p_assigned_to IS NULL OR assigned_to = p_assigned_to)
            GROUP BY 1
        ) w ON w.month_date = m.month_date
    )
    SELECT json_build_object(
        'stats', json_build_object(
            ...
            'totalWonPotential', v_curr_won_potential,
        ...
        'byStatus', (
            ...
            FROM (
                ...
                UNION ALL
                SELECT status, COUNT(*) as count, COALESCE(SUM(COALESCE(closing_amount, value, 0)), 0) as amount FROM public.leads WHERE company_id = p_company_id
                  AND (p_assigned_to IS NULL OR assigned_to = p_assigned_to)
        ...
        'salesTrend', (SELECT COALESCE(json_agg(json_build_object('date', day, 'amount', amount, 'count', count, 'conversionRate', COALESCE(conversion_rate, 0))), '[]'::json) FROM (
            SELECT 
                DATE_TRUNC('day', l.internal_won_date)::DATE as day, 
                COALESCE(SUM(COALESCE(l.closing_amount, l.value, 0)), 0) as amount, 
                COUNT(*) as count,
                MAX(ROUND((COALESCE(mc.won_leads, 0)::NUMERIC / NULLIF(mc.total_leads, 0)) * 100, 1)) as conversion_rate
            FROM public.leads l
            LEFT JOIN mc ON mc.month_date = DATE_TRUNC('month', l.internal_won_date)::DATE
            WHERE l.company_id = p_company_id AND l.status IN ('Cerrado', 'Cliente') AND l.internal_won_date IS NOT NULL
              AND (p_assigned_to IS NULL OR l.assigned_to = p_assigned_to)
              AND (p_start_date IS NULL OR l.internal_won_date >= p_start_date) 
              AND (p_end_date IS NULL OR l.internal_won_date <= p_end_date) 
        ...
        'salesKpis', (SELECT COALESCE(json_agg(json_build_object('agent_name', p.full_name, 'actual', COALESCE(s.actual_sales, 0), 'target', COALESCE(g.target, 0),
            ...
            LEFT JOIN (SELECT assigned_to, SUM(COALESCE(closing_amount, value, 0)) as actual_sales FROM public.leads WHERE company_id = p_company_id AND status IN ('Cerrado', 'Cliente')
              AND (p_assigned_to IS NULL OR assigned_to = p_assigned_to)
