-- =====================================================
-- MIGRATION: Add Lost Leads Tracking System (PRODUCTION SAFE)
-- =====================================================
...
-- Add columns to leads table (SAFE: only adds, never modifies)
ALTER TABLE leads 
...
-- Enable RLS on loss_reasons (safe if already enabled)
ALTER TABLE loss_reasons ENABLE ROW LEVEL SECURITY;

-- RLS Policies (wrapped in DO block to handle "already exists" errors gracefully)
DO $$
BEGIN
    -- Policy: Users can view their company's loss reasons + templates
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'loss_reasons' 
        AND policyname = 'Users can view their company loss reasons and templates'
    ) THEN
    ...
    -- Policy: Only admins can manage loss reasons
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'loss_reasons' 
        AND policyname = 'Admins can manage loss reasons'
    ) THEN
...
-- MIGRATION COMPLETE
-- =====================================================
-- ✅ New table 'loss_reasons' created
-- ✅ 4 new columns added to 'leads' table
-- ✅ RLS policies configured
-- ✅ Auto-copy trigger for new companies
-- ✅ Default loss reasons inserted
-- =====================================================
