...
import { X, Mic, Send, Calendar, PlayCircle, MessageSquare } from 'lucide-react';
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {!isOpen && (
        <button 
          ...
          className="relative group flex items-center gap-3 bg-white p-2 pr-4 rounded-full shadow-lg border border-slate-200 hover:shadow-xl transition-all duration-300 animate-in slide-in-from-bottom-5"
        >
          <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-white shadow-sm shrink-0">
            <img src="/arias_agent_avatar.png" alt="Arias" className="w-full h-full object-cover object-top" />
          </div>
          <div className="flex flex-col items-start">
            <span className="text-sm font-bold text-slate-800">Speak with Arias</span>
            <span className="text-xs font-medium text-slate-500">AI SDR Agent</span>
          </div>
