...
export default function Sidebar({ isCollapsed, onToggle }: { isCollapsed: boolean, onToggle: () => void }) {
    ...
    useEffect(() => {
        const isEligible = profile?.role === 'super_admin' || 
                           (profile as any)?.is_platform_owner === true ||
    ...
    return (
        <div className={cn(
            ...
            <div className={cn("relative flex flex-col items-center justify-center min-h-[6.5rem] border-b px-4 py-2", theme.border, theme.headerBg)}>
                ...
                {!isCollapsed && (profile?.role === 'super_admin' || (profile as any)?.is_platform_owner === true || profile?.email?.toLowerCase() === 'jarias7604@gmail.com' || profile?.email?.toLowerCase() === 'jarias@ariasdefense.com') && (
