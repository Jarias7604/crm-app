...
export default function ChatHub() {
    ...
    const handleSendPendingQuote = async () => {
        ...
        try {
            // Send ONLY the PDF as a real file attachment as requested
            // We skip the __QUOTE__ card because the user doesn't want links leading to login
            if (pendingQuote.pdfUrl) {
                toast.loading('Enviando propuesta PDF...', { id: 'send-pdf' });
                await chatService.sendMessage(
                    selectedConv.id,
                    `📄 Propuesta Comercial: ${pendingQuote.nombre_cliente}.pdf`,
                    'file',
                    {
                        url: pendingQuote.pdfUrl,
                        fileName: `Propuesta_${pendingQuote.nombre_cliente}.pdf`,
                        isAutoGenerated: true
                    }
                );
                toast.success('¡PDF enviado correctamente!', { id: 'send-pdf' });
            } else {
                // Fallback if no PDF (should not happen with new logic)
                const messageContent = `Propuesta de Inversión: $${pendingQuote.total_anual.toLocaleString()} para ${pendingQuote.nombre_cliente}`;
