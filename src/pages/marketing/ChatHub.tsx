...
export default function ChatHub() {
    ...
    useEffect(() => {
        if (selectedConv && selectedConv.id !== 'new') {
            ...
            const sub = chatService.subscribeToMessages(selectedConv.id, async (msg) => {
                setMessages(prev => {
                ...
                if (msg.direction === 'outbound' && msg.metadata?.quote_trigger) {
                    ...
                    console.log('Hub: Auto-generating PDF from trigger detection...');

                    try {
                        const trigger = msg.metadata.quote_trigger;
                        const result = await aiQuoteService.processAiQuote({
                            ...trigger,
                            lead_id: selectedConv.lead?.id || '',
                            conversation_id: selectedConv.id,
                            company_id: profile?.company_id || ''
                        });

                        // Send the PDF automatically to the client
                        if (result.success && result.pdfUrl) {
                            // 1. Mark as dispatched in DB first to prevent other tabs from racing
                            await chatService.updateMessageMetadata(msg.id, { pdf_dispatched: true });

                            // 2. Send the message
                            await chatService.sendMessage(
                                selectedConv.id,
                                `¡Listo! He preparado la propuesta oficial para un volumen de ${trigger.dte_volume?.toLocaleString()} DTEs. Adjunto encontrarás el PDF con todos los detalles técnicos y comerciales.`,
                                'file',
                                'outbound',
                                { url: result.pdfUrl, fileName: 'Propuesta_Comercial.pdf', isAutoGenerated: true }
                            );
                            toast.success('Cotización enviada automáticamente por el asistente');
                        }
                    } catch (e) {
                        console.error('Auto-quote auto-delivery error:', e);
                        processedTriggers.current.delete(msg.id); // Allow retry on failure
                    }
                }
            });
            return () => { sub.unsubscribe(); };
    ...
    }, [selectedConv?.id, agentStatus, profile?.company_id]);
