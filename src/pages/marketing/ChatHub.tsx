...
export default function ChatHub() {
    ...
    const handleSendPendingQuote = async () => {
        ...
        try {
            ...
            if (pendingQuote.pdfUrl) {
                toast.loading('Enviando propuesta PDF...', { id: 'send-pdf' });

                // Use the centralized PDF Service to ensure consistency
                // If pdfUrl is already a storage URL, we might want to regenerate to be sure it's fresh?
                // For now, let's assume if it came from the builder it might be a blob or old.
                // Safest to regenerate using the exact same service logic.
                const { pdfService } = await import('../../services/pdfService');
                const freshUrl = await pdfService.generateAndUploadQuotePDF(pendingQuote);
                ...
                await chatService.sendMessage(
                    ...
                    {
                        url: freshUrl,
                        fileName: `Propuesta_${pendingQuote.nombre_cliente}.pdf`,
                ...
                // Fallback (text only)
                const leadName = selectedConv.lead?.name || pendingQuote.nombre_cliente || 'Cliente';
                const messageContent = `Hola ${leadName}, aquí tienes el resumen de tu propuesta de inversión: $${pendingQuote.total_anual.toLocaleString()}`;
                await chatService.sendMessage(selectedConv.id, messageContent);
                toast.success('¡Resumen enviado!', { id: 'send-pdf' });
            }
