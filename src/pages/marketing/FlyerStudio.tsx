...
export default function FlyerStudio() {
  ...
  async function generate(mode: 'full' | 'background' = 'background') {
    ...
    try {
      ...
      setSelected(0);
      setPreviewMode('ai');
      toast.success('✨ Flyer generado usando imagen temática premium (Modo Demo)');
    } finally {
      setGenerating(false);
    }
  }
        ...
        <div style={css.col('340px')}>
          ...
          <div className="flyer-studio-col" style={css.colBody}>
            ...
            <div style={{ paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <button onClick={() => generate('full')} disabled={generating || !prompt.trim()}
                ...
                {generating && isFullAiFlyer ? (
                  <><Cpu size={14} style={{ animation: 'spin 1s linear infinite' }} /> Generando Flyer con IA...</>
                ) : (
                  <><Sparkles size={14} color="#D4AF37" fill="#D4AF37" /> Generar Flyer con IA (Completo) <ChevronRight size={13} /></>
                )}
              </button>

              {/* Alternative generate button for clean background only */}
              <button onClick={() => generate('background')} disabled={generating || !prompt.trim()}
                style={{ background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: 8, padding: '10px 20px', fontSize: 11, fontWeight: 700, color: '#475569', cursor: generating || !prompt.trim() ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 6, width: '100%', justifyContent: 'center', opacity: generating || !prompt.trim() ? 0.6 : 1 }}>
                {generating && !isFullAiFlyer ? (
                  <><Cpu size={12} style={{ animation: 'spin 1s linear infinite' }} /> Generando Fondo...</>
                ) : (
                  <><Image size={12} color="#6366f1" /> Generar Solo Fondo con IA (Alternativo)</>
                )}
              </button>
              <style>{`
              ...
              <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {true ? (
                  <div style={{ display: 'flex', justifyContent: 'center', width: '100%', userSelect: 'none' }}>
                    ...
                    >
                      {bgUploadPreview || (isFullAiFlyer && variants.length > 0) ? (
                        <img
