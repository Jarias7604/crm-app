...
export default function MarketingDashboard() {
    ...
    const handleFilterRedirect = (type: 'sent' | 'opens' | 'clicks') => {
        let filteredIds: string[] = [];

        if (type === 'sent') {
            filteredIds = heatmapLeads.map(l => l.id);
        } else if (type === 'opens') {
            filteredIds = heatmapLeads.filter(l => l.opens > 0).map(l => l.id);
        } else if (type === 'clicks') {
            filteredIds = heatmapLeads.filter(l => l.clicks > 0).map(l => l.id);
        }

        if (filteredIds.length > 0) {
            navigate('/leads', { state: { leadIds: filteredIds } });
        }
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white p-6 rounded-3xl shadow-sm border border-gray-100 min-h-[400px]">
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex-1">
                            ...
                            <div className="flex items-center gap-4 mt-1">
                                <p className="text-sm text-gray-500">Leads con mayor interacción histórica.</p>
                                <div className="h-4 w-[1px] bg-gray-200 mx-1"></div>
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={() => handleFilterRedirect('sent')}
                                        className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded-lg text-[10px] font-black uppercase hover:bg-gray-200 transition-colors"
                                    >
                                        • {heatmapLeads.reduce((acc, l) => acc + l.sent, 0)} IMPACTOS
                                    </button>
                                    <button
                                        onClick={() => handleFilterRedirect('opens')}
                                        className="px-2 py-0.5 bg-orange-50 text-orange-600 rounded-lg text-[10px] font-black uppercase hover:bg-orange-100 transition-colors"
                                    >
                                        • {heatmapLeads.reduce((acc, l) => acc + l.opens, 0)} APERTURAS
                                    </button>
                                    <button
                                        onClick={() => handleFilterRedirect('clicks')}
                                        className="px-2 py-0.5 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-black uppercase hover:bg-blue-100 transition-colors"
                                    >
                                        • {heatmapLeads.reduce((acc, l) => acc + l.clicks, 0)} CLICKS
                                    </button>
                                </div>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <button className="p-2 hover:bg-gray-100 rounded-xl text-gray-400 transition-all border border-transparent hover:border-gray-200">
                                <Settings className="w-4 h-4" />
                            </button>
                            <Link to="/marketing/email" className="text-blue-600 text-sm font-bold hover:underline flex items-center gap-1 group">
                                Ver todos
                    ...
                    <div className="space-y-4">
                        {/* Heatmap List */}
                        <div className="overflow-y-auto max-h-[450px] custom-scrollbar rounded-2xl border border-gray-100 bg-white/50 shadow-inner">
