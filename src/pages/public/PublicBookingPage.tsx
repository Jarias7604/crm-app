...
export default function PublicBookingPage() {
    ...
    const txt = (esVal: string, enVal: string) => isSpanish ? esVal : enVal;
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.error(txt('Error al agendar. Por favor intenta de nuevo.', 'Error scheduling. Please try again.'));
        } finally {
            setSubmitting(false);
        }
    };
    ...
    if (notFound || !link) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-white text-center p-8">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">{txt('Enlace no encontrado', 'Link not found')}</h1>
            <p className="text-gray-400">{txt('Este enlace de reserva no existe o ha sido desactivado.', 'This booking link does not exist or has been disabled.')}</p>
        </div>
    );
            ...
            {step === 'confirmed' ? (
                <div className="flex-1 flex items-center justify-center p-6">
                    <div className="bg-white rounded-[32px] shadow-2xl border border-gray-100 p-8 md:p-12 max-w-lg w-full text-center animate-in zoom-in-95 duration-200">
                        ...
                        <h2 className="text-2xl font-black text-gray-900 mb-2">{txt('¡Reunión Confirmada!', 'Meeting Confirmed!')}</h2>
                        <p className="text-sm text-gray-400 mb-8 font-medium">{txt('Se ha enviado una confirmación con todos los detalles a tu correo electrónico.', 'A confirmation has been sent with all details to your email address.')}</p>
                        ...
                        <div className="space-y-3 mb-8 border-t border-b border-gray-100 py-6 text-left">
                            <h4 className="text-[10px] font-black uppercase text-gray-400 tracking-widest mb-3 text-center">{txt('Añadir a mi Calendario', 'Add to my Calendar')}</h4>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                ...
                                <a 
                                    ...
                                    })()}&details=${encodeURIComponent(link.description || txt('Reunión agendada vía web.', 'Meeting scheduled via web.'))}&location=${encodeURIComponent(link.location || txt('Videollamada', 'Video call'))}`}
                                    target="_blank" 
                                ...
                                <a 
                                    ...
                                    })()}&body=${encodeURIComponent(link.description || txt('Reunión agendada vía web.', 'Meeting scheduled via web.'))}&location=${encodeURIComponent(link.location || txt('Videollamada', 'Video call'))}`}
                                    target="_blank" 
                                    rel="noopener noreferrer"
                            ...
                            <button 
                                onClick={() => {
                                    ...
                                    const icsContent = [
                                        ...
                                        `DESCRIPTION:${link.description || txt('Reunión agendada vía web.', 'Meeting scheduled via web.')}`,
                                        `LOCATION:${link.location || txt('Videollamada', 'Video call')}`,
                                        'END:VEVENT',
                                        'END:VCALENDAR'
                                    ].join('\r\n');
                                    ...
                                    toast.success(txt('¡Archivo de cita descargado!', 'Meeting file downloaded!'));
                                }}
                            ...
                            >
                                ...
                                {txt('Descargar Archivo de Cita (.ics)', 'Download Appointment File (.ics)')}
                            </button>
                        </div>
                        ...
                        <button onClick={() => { setStep('calendar'); setSelectedDate(null); setSelectedSlot(null); setForm({ name:'',email:'',phone:'',company:'',notes:'' }); }}
                            ...
                            {txt('Agendar otra reunión', 'Schedule another meeting')}
                        </button>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden max-w-xl w-full">
                        <div className="px-8 py-5 border-b border-gray-100 flex items-center gap-4">
                            ...
                            <div className="flex-1">
                                <p className="text-xs text-gray-400 font-medium">{txt('Confirmar tu reunión', 'Confirm your meeting')}</p>
                                <p className="text-base font-semibold text-gray-900">
                                    {selectedDate && (isSpanish 
                                        ? format(selectedDate, "EEEE dd 'de' MMMM", { locale: es }) 
                                        : format(selectedDate, "EEEE, MMMM dd", { locale: enUS }))} · {selectedSlot && format(new Date(selectedSlot), 'h:mm a')}
                                </p>
                            </div>
                        </div>
                        <div className="p-8 space-y-5">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <InputField icon={User} label={txt('Nombre completo *', 'Full name *')} placeholder={txt('Tu nombre', 'Your name')} value={form.name} onChange={v => setForm(f => ({...f, name: v}))} />
                                <InputField icon={Mail} label={txt('Correo electrónico *', 'Email address *')} placeholder={txt('correo@empresa.com', 'email@company.com')} value={form.email} onChange={v => setForm(f => ({...f, email: v}))} type="email" />
                                <InputField icon={Phone} label={txt('Teléfono', 'Phone number')} placeholder="+503 0000-0000" value={form.phone} onChange={v => setForm(f => ({...f, phone: v}))} />
                                <InputField icon={Building2} label={txt('Empresa', 'Company')} placeholder={txt('Nombre de tu empresa', 'Your company name')} value={form.company} onChange={v => setForm(f => ({...f, company: v}))} />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-gray-500 mb-1.5">{txt('Notas adicionales', 'Additional notes')}</label>
                                <textarea rows={3} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all resize-none placeholder:text-gray-300"
                                    placeholder={txt('¿De qué tema quieres hablar en la reunión?', 'What topic would you like to discuss in the meeting?')} value={form.notes} onChange={e => setForm(f => ({...f, notes: e.target.value}))} />
                            </div>
                            <button onClick={handleSubmit} disabled={!form.name || !form.email || submitting}
                                ...
                                {txt('Confirmar Reunión', 'Confirm Meeting')}
                            </button>
                            <div className="flex items-center justify-center gap-1.5 text-xs text-gray-300">
                                <Shield className="w-3 h-3" />
                                <span>{txt('Comunicación segura y confidencial', 'Secure and confidential communication')}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 flex items-center justify-center px-4 py-4 md:py-6">
                    <div className="w-full" style={{ maxWidth: '1012px' }}>
                        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                            <div className="flex flex-col md:flex-row">
                                ...
                                <div className="md:w-[253px] shrink-0 py-6 px-5 border-b md:border-b-0 md:border-r border-gray-100 flex flex-col">
                                    ...
                                    {/* Agent Photo */}
                                    {link.avatar_url ? (
                                        <img src={link.avatar_url} className="w-[62px] h-[62px] rounded-full object-cover mb-3.5 shadow-sm" alt={link.display_name} />
                                    ) : (
                                        <div className="w-[62px] h-[62px] rounded-full flex items-center justify-center text-white text-2xl font-bold mb-3.5 shadow-sm" style={{ backgroundColor: brandColor }}>
                                    ...
                                    {link.description ? (
                                        ...
                                        <p className="mt-4 text-xs text-gray-400 leading-relaxed border-t border-gray-100 pt-3">
                                            {txt(
                                                ...
                                                'Schedule a free meeting to discuss your needs. No commitments, no pressure — just a professional conversation.'
                                            )}
                                        </p>
                                    )}
                                    ...
                                    <div className="mt-auto pt-4">
                                        <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl p-2.5 flex flex-col gap-1">
                                            <div className="flex items-center gap-1 text-[10px] text-emerald-800 font-black">
                                                ...
                                                <span>{txt('CONEXIÓN 100% SEGURA', '100% SECURE CONNECTION')}</span>
                                            </div>
                                            <p className="text-[9px] text-emerald-600 leading-relaxed font-medium">
                                                {txt(
                                                    ...
                                                    'Portal verified by Arias CRM. Data and video call encrypted SSL end-to-end.'
                                                )}
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div className="flex-1 py-6 px-5 md:py-8 md:px-6 border-b md:border-b-0 md:border-r border-gray-100">
                                    <h2 className="text-base font-bold text-gray-900 mb-3">{txt('Selecciona fecha y hora', 'Select date and time')}</h2>

                                    {/* Month navigation */}
                                    <div className="flex items-center justify-between mb-4">
                                        <span className="text-sm font-semibold text-gray-900">{months[currentMonth.getMonth()]} {currentMonth.getFullYear()}</span>
                                    ...
                                    <div className="flex items-center gap-1.5 mt-5 text-[10px] text-gray-400">
                                        ...
                                        <span>{txt('Hora Central (El Salvador, CST)', 'Central Time (El Salvador, CST)')}</span>
                                    </div>
                                </div>
                                ...
                                <div className="md:w-[220px] shrink-0 py-6 px-5 flex flex-col justify-start">
                                    {!selectedDate ? (
                                        <div className="flex flex-col items-center justify-center h-full text-center py-16">
                                            ...
                                            <p className="text-xs text-gray-300 font-medium">{txt('Selecciona un día', 'Select a day')}</p>
                                            <p className="text-[10px] text-gray-200 mt-0.5">{txt('para ver horarios', 'to view times')}</p>
                                        </div>
                                    ) : (
                                        <>
                                            ...
                                            {slots.length > 0 && !loadingSlots && (
                                                <div className="inline-flex items-center gap-1.5 mb-3 bg-emerald-50 text-emerald-800 px-2.5 py-1 rounded-lg border border-emerald-100/50 self-start">
                                                    ...
                                                    <span className="text-[9px] font-black uppercase tracking-wider">{txt('disponibles', 'available')}</span>
                                                </div>
                                            )}
                                            ...
                                            {loadingSlots ? (
                                                ...
                                                <div className="text-center py-12">
                                                    <p className="text-xs text-gray-400 font-medium">{txt('Sin horarios', 'No slots available')}</p>
                                                    <p className="text-[10px] text-gray-300 mt-0.5">{txt('Elige otro día', 'Choose another day')}</p>
                                                </div>
                                            ) : (
                                                            ...
                                                            >
                                                                {active ? (
                                                                    <span className="flex items-center justify-center gap-2">
                                                                        ...
                                                                        <span className="text-[9px] bg-white/20 px-2 py-0.5 rounded-full font-bold">{txt('Confirmar', 'Confirm')}</span>
                                                                    </span>
                                                                ) : (
